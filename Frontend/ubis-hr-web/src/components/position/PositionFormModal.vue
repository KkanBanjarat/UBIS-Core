        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="font-semibold text-base leading-tight">
              {{ isCreate ? 'เพิ่มตำแหน่งงาน' : 'แก้ไขตำแหน่งงาน' }}
            </h3>
        ...
        <div class="overflow-y-auto px-6 py-5 space-y-4 flex-1">
          <div>
            <label class="block text-xs font-medium text-base-content/60 mb-1.5">
              ชื่อตำแหน่ง (ไทย) <span class="text-error">*</span>
            </label>
            <input
          ...
          <div>
            <label class="block text-xs font-medium text-base-content/60 mb-1.5">
              ชื่อตำแหน่ง (อังกฤษ) <span class="text-error">*</span>
            </label>
            <input
...
const validationSchema = yup.object({
  nameTh: yup.string().required('กรุณาระบุชื่อตำแหน่งภาษาไทย'),
  nameEn: yup.string().required('กรุณาระบุชื่อตำแหน่งภาษาอังกฤษ'),
})

function open(item?: Position | null) {
  errors.value = {}
  formData.value = item
    ? {
        nameTh: item.nameTh,
